...
3. **Assign cycle** (`src/lib/assign.ts → autoAssignCycle`):
   ...
   - Day-boundary rollover: a once-per-day morning pass (Redis-gated, runs before the fetch on the first armed cycle) reclaims yesterday's unfinished ad-hoc jobs into today. It fetches yesterday's status 2 (unassigned) + status 4 (assigned-but-unfinished, incl. started), keeps only jobs with **no plan attached** (`hasPlanAttached` — plan slots regenerate daily, so rolling one would duplicate it) and no **engine-leg label** (`isEngineLeg` / `ENGINE_LEG_LABELS` — outbound, via and return belong to `cleanupStaleTrips`, which only finds them while still dated yesterday; a rolled leg loses its driver and becomes unremovable), unassigns any stale driver, and re-dates `scheduled_delivery_ts` to today so the same cycle assigns them. `rolloverUnfinishedJobs` in `assign.ts`
...
| `src/lib/job-filters.ts` | Edge-safe job/stop vocabulary (imports only `time.ts`). `JOB_STATUS`, `STOP_STATUS` maps; `isActiveStop`, `isCompletedOrRejectedStop`, `isStopStarted`, `isBlockingPickupStop`; the note gate (`isBlockingNote`, `blockingNotes`, `DAYTIME_NON_BLOCKING`, `NOTE_RELEASE_CUTOFF_MIN`); `isChamCong`; `pscPairKey`; and the PSC engine-leg labels — `PSC_OUTBOUND_LABEL`, `PSC_VIA_LABEL`, `PSC_RETURN_LABEL`, with `ENGINE_LEG_LABELS` / `isEngineLeg` over them |
