...
| `POST\|PUT /api/schedule-job/row` | Add / edit a Lịch cố định row in the schedule sheet (service-account write; PUT re-checks `rowIndex` against `original.reference`+`pickup_id`). Optional `driver_id` → sheet `driver`/`driver_id` columns (auto-added if missing): at release from the proxy, `releaseDueProxyJobs` assigns the job straight to that driver unless they're on leave or the assign fails, then it releases normally |
