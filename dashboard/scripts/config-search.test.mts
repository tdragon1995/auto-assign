...
console.log("dashboard phrase and facet filters");
{
  ...
  eq("pickup is an exact phrase", hits({ pickupOperator: "is", pickupText: "kho miền đông" }), [32]);
  eq("pickup is not an exact phrase", hits({ pickupOperator: "is_not", pickupText: "kho miền đông" }), [30, 31, 33]);
  ...
  eq("drop-off is exact", hits({ dropoffOperator: "is", dropoffText: "phòng khám an bình" }), [33]);
  eq("drop-off is not exact", hits({ dropoffOperator: "is_not", dropoffText: "phòng khám an bình" }), [30, 31, 32]);
  ...
  eq("driver is matches one full name in a smart row", hits({ driverOperator: "is", driverText: VIET_PHI }), [31, 32]);
  eq("driver is matches the displayed personal name", hits({ driverOperator: "is", driverText: "Nguyễn Viết Phi" }), [31, 32]);
  eq("driver is not excludes a smart row if any full name matches", hits({ driverOperator: "is_not", driverText: VIET_PHI }), [30, 33]);
  eq("driver is not excludes the displayed personal name", hits({ driverOperator: "is_not", driverText: "Nguyễn Viết Phi" }), [30, 33]);
  eq("empty text does not activate a negative operator", hits({ driverOperator: "not_contains" }), [30, 31, 32, 33]);
  eq("separate controls combine with AND", hits({ drivers: [VIET_PHI], pickupText: "miền", dropoffs: [""] }), [32]);
