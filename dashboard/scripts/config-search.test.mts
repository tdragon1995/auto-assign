...
console.log("dashboard phrase and facet filters");
{
  ...
  eq("default driver filter selects exact names", hits({ drivers: [VIET_PHI] }), [31, 32]);
  eq("default shift filter selects exact times", hits({ starts: ["13:00"] }), [31]);
  ...
  eq("pickup contains is accent-insensitive", hits({ pickupOperator: "contains", pickupText: "mien dong" }), [32]);
  ...
  eq("drop-off contains is accent-insensitive", hits({ dropoffOperator: "contains", dropoffText: "phong kham" }), [33]);
  eq("drop-off does not contain", hits({ dropoffOperator: "not_contains", dropoffText: "bệnh viện" }), [32, 33]);
  eq("driver contains one name in a smart row", hits({ driverOperator: "contains", driverText: "viết phi" }), [31, 32]);
  ...
  eq("shift start contains narrows to one time", hits({ startOperator: "contains", startText: "13:00" }), [31]);
