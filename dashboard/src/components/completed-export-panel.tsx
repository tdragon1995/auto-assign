...
// Column header ↔ row key, in order. Names stay faithful to Cartrack. The raw
...
/**
 * Completed-jobs export ("Distance checking" tab): pull every job_status_id=5
 * job in a scheduled_delivery_ts date range, optionally enrich with Goong
 * mileage, and download a real .xlsx whose sheet is named "Export".
 * Layout-agnostic — the caller wraps it (a scroll container in the dashboard
 * tab, a full-page wrapper on the standalone route).
 */
export function CompletedExportPanel({ env = "prod" }: { env?: Env }) {
  ...
  const load = async () => {
    ...
    try {
      const res = await fetch(`/api/export-completed?from=${from}&to=${to}&env=${env}`);
  ...
  return (
    <div className="p-3 sm:p-4 space-y-4">
      <div>
        <h2 className="text-lg font-bold">Distance checking — chuyến đã hoàn thành</h2>
